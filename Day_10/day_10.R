...
dat <- readLines("Day_10/day_10_dat.txt") %>% 
  as.integer() %>% 
  sort()
dat <- c(dat, max(dat)+ 3) # Adding in for the final in device adaptor 
# Part 1 
tibble(adapter = dat, 
       diff = dat - lag(dat, default = 0)) %>% 
  filter(diff < 4) %>% 
  summarise(diff_1 = sum(diff == 1),
            diff_3 = sum(diff == 3)) %>% 
  mutate(answer = diff_1 * diff_3)

# Part 2
# Get the location of numbers which are dropable 
result <- tibble(adapter = dat, 
       diff = dat - lag(dat, default = 0),
       req = diff == 3, 
...
part_2 <- result %>% 
  ...
  mutate(n_combs = case_when(seq_length == 1 ~ 2^1, # there or not 
                             seq_length == 2 ~ 2^2, # every comb of being there 
                             seq_length == 3 ~ 2^3 - 1 , # remove 1 because all 3 can't be missing 
                             seq_length == 4 ~ 2^4 - 4 )) %>%# remove the counts that would violate the rules
